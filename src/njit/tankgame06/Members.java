...
class Recorder {
	...
	public static void save() {
		...
		fileChooser.setFileFilter(new FileExtensionFilter("文本文件(*.txt)","txt"));
		...
		try {
			...
			bufferedWriter.write(Global.enemyNumber + " " + Global.heroNumber +" " + deadEnemy +"\r\n");
	...
	public static Vector<Node> loadGame() {
		...
		try {
			...
			Global.enemyNumber = Integer.parseInt(aNode[0]);
			Global.heroNumber = Integer.parseInt(aNode[1]);
			deadEnemy = Integer.parseInt(aNode[2]);
			
			while((aLine=bufferedReader.readLine()) != null)
			{
				aNode = aLine.split(" "); 				
				Node node=new Node(Integer.parseInt(aNode[0]),Integer.parseInt(aNode[1]),Direct.parse(aNode[2]));
				nodes.add(node);
			}
