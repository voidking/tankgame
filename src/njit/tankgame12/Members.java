...
class Recorder {
	...
	public static void save() {
		...
		try {
			...
			bufferedWriter.write(Recorder.enemyNumber + " "
					+ Recorder.heroNumber + " " + deadEnemy + "\r\n");
	...
	public static void saveSet() {
		try {
			FileOutputStream set = new FileOutputStream("D:\\Set.txt");
			byte buf[] = { (byte) Global.enemyNumber, (byte) Global.enemySpeed };
			set.write(buf);
			set.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void loadSet() {
		try {
			FileInputStream set = new FileInputStream("D:\\Set.txt");
			byte[] buf = new byte[1024];
			set.read(buf);
			Global.enemyNumber = (int) buf[0];
			Recorder.enemyNumber = (int) buf[0];
			Global.enemyBulletSpeed = (int) buf[1];
			set.close();
		} catch (Exception e) {
			e.printStackTrace();
		}
