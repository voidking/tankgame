...
public class TankGame extends JFrame implements ActionListener {
	...
	public TankGame() {
		...
		JMenuItem saveSet = new JMenuItem("保存设置");
		saveSet.addActionListener(this);
		saveSet.setActionCommand("saveSet");
		menu.add(saveSet);
		
		JMenuItem loadSet = new JMenuItem("读取设置");
		loadSet.addActionListener(this);
		loadSet.setActionCommand("loadSet");
		menu.add(loadSet);
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if(e.getActionCommand().equals("saveSet"))
		{
			Recorder.saveSet();
		} else if(e.getActionCommand().equals("loadSet"))
		{
			Recorder.loadSet();
		} else if(e.getActionCommand().equals("back"))
		{
			if(addPanelFlag)
			{
				Recorder.deadEnemy = 0;
				Recorder.heroNumber = Global.heroNumber;
				Recorder.enemyNumber = Global.enemyNumber;
...
class MyPanel extends JPanel implements KeyListener, Runnable {
	...
	public MyPanel(String flag) {
		...
		if (flag.equals("newGame")) {
			...
			hero = new Hero(Global.heroX, Global.heroY, Direct.UP,
					Global.heroColor, Global.heroSpeed);
			// 初始化敌人的坦克
			for (int i = 0; i < nodes.size(); i++) {
				Node node = nodes.get(i);

				Enemy enemy = new Enemy(node.x, node.y, node.direct,
						Global.enemyColor, Global.enemySpeed);
				Thread enemyThread = new Thread(enemy);
				enemyThread.start();
				// 给敌人坦克添加一颗子弹
				Bullet enemyBullet = new Bullet(enemy.x + 10, enemy.y + 30,
						enemy.direct, Global.enemyBulletSpeed);
				enemy.bullets.add(enemyBullet);
				Thread bulletThread = new Thread(enemyBullet);
				bulletThread.start();
				enemyVector.add(enemy);
	...
	public void paint(Graphics g) {
		...
		if (paintFlag.equals("newGame") || paintFlag.equals("loadGame")) {
			...
			for (int i = 0; i < heros.size(); i++) {
				...
				if (otherHero.live) {
					g.drawString("ID:" + otherHero.id, otherHero.x - 10,
							otherHero.y - 10);
					otherHero.drawTank(g);
					for (int j = 0; j < otherHero.bullets.size(); j++) {
						// 取出子弹
						Bullet tempBullet = otherHero.bullets.get(j);
						if (tempBullet.live == true) {
							g.draw3DRect(tempBullet.x, tempBullet.y, 1, 1, false);
						}else if (tempBullet.live == false) {
							otherHero.bullets.remove(tempBullet);
						}
					}
				}
	...
	public void showInfo(Graphics g) {
		...
		g.drawString(Recorder.enemyNumber + "", 40, Global.panelHeight + 30);

		Tank heroTank = new Tank(70, Global.panelHeight + 10, Direct.UP,
				Global.heroColor, 0);
		heroTank.drawTank(g);
		g.setColor(Color.black);
		g.drawString(Recorder.heroNumber + "", 100, Global.panelHeight + 30);
	...
	// 判断其他用户是否击中我
	public void otherHitMe() {
		if (this.hero.live == true) {
			// 取出每一个敌人坦克
			for (int i = 0; i < this.heros.size(); i++) {
				// 取出坦克
				Hero temHero = heros.get(i);
				// 取出每一颗子弹
				for (int j = 0; j < temHero.bullets.size(); j++) {
					// 取出子弹
					Bullet b = temHero.bullets.get(j);
					this.hit(b, hero);
				}
			}
		}
	...
	// 判断其他用户是否击中其他用户
	public void otherHitOther() {
		for (int i = 0; i < this.heros.size(); i++) {
			// 取出坦克
			Hero tempHero = heros.get(i);
			// 取出每一颗子弹
			for (int j = 0; j < tempHero.bullets.size(); j++) {
				// 取出子弹
				Bullet b = tempHero.bullets.get(j);
				for(int k = 0;k < this.heros.size();k++)
				{
					Hero tempHero2 = this.heros.get(k);
					if(tempHero.id != tempHero2.id)
					{
						this.hit(b, tempHero2);
					}
				}
		...
		for (int i = 0; i < hero.bullets.size(); i++) {
			...
			if (b.live == true) {
				for (int j = 0; j < heros.size(); j++) {
					Hero tempHero = heros.get(j);
					if (tempHero.live == true) {
						if (this.hit(b, tempHero)) {
							// 其他用户坦克消失
							tempHero.live = false;
						}
					}
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_J) {
			// 当按下“J”时，坦克发射子弹
			if (this.hero.live
					&& this.hero.bullets.size() < Global.heroBulletNumber) {
...
class SetFrame extends JFrame implements ActionListener {
	...
	Color realColors[] = { Color.red, Color.orange, Color.yellow, Color.green,
	...
	public void actionPerformed(ActionEvent e) {
		...
		Recorder.heroNumber = (int) comboHeroNumber.getSelectedItem();
		...
		Recorder.enemyNumber = (int) comboEnemyNumber.getSelectedItem();
		Global.enemyBulletNumber = (int) comboEnemyBulletNumber
				.getSelectedItem();
